...
fn byte_sub(state: &mut [[u8; 4]; 4]) {
    for row in state.iter_mut().take(4) {
        for item in row.iter_mut().take(4) {
            let ms_nibble: usize = ((*item & 0xF0) >> 4).into();
            let ls_nibble: usize = (*item & 0x0F).into();

            *item = S_BOX[ms_nibble][ls_nibble];
        }
    }
}
...
fn inv_byte_sub(state: &mut [[u8; 4]; 4]) {
    for _row in state.iter_mut().take(4) {
        for item in _row.iter_mut().take(4) {
            let ms_nibble: usize = ((*item & 0xF0) >> 4).into();
            let ls_nibble: usize = (*item & 0x0F).into();

            *item = S_BOX_INV[ms_nibble][ls_nibble];
        }
    }
}
...
fn mix_cols(state: &mut [[u8; 4]; 4]) {
    let mut new_state: [[u8; 4]; 4] = *state;
    for col in 0..4 {
        new_state[0][col] = TABLE_2[state[0][col] as usize]
            ^ TABLE_3[state[1][col] as usize]
            ^ state[2][col]
            ^ state[3][col];
        new_state[1][col] = state[0][col]
            ^ TABLE_2[state[1][col] as usize]
            ^ TABLE_3[state[2][col] as usize]
            ^ state[3][col];
        new_state[2][col] = state[0][col]
            ^ state[1][col]
            ^ TABLE_2[state[2][col] as usize]
            ^ TABLE_3[state[3][col] as usize];
        new_state[3][col] = TABLE_3[state[0][col] as usize]
            ^ state[1][col]
            ^ state[2][col]
            ^ TABLE_2[state[3][col] as usize];
    }
...
#[allow(dead_code)]
fn inv_mix_cols(state: &mut [[u8; 4]; 4]) {
    let mut new_state: [[u8; 4]; 4] = *state;
    for col in 0..4 {
        new_state[0][col] = TABLE_14[state[0][col] as usize]
            ^ TABLE_11[state[1][col] as usize]
            ^ TABLE_13[state[2][col] as usize]
            ^ TABLE_9[state[3][col] as usize];
        new_state[1][col] = TABLE_9[state[0][col] as usize]
            ^ TABLE_14[state[1][col] as usize]
            ^ TABLE_11[state[2][col] as usize]
            ^ TABLE_13[state[3][col] as usize];
        new_state[2][col] = TABLE_13[state[0][col] as usize]
            ^ TABLE_9[state[1][col] as usize]
            ^ TABLE_14[state[2][col] as usize]
            ^ TABLE_11[state[3][col] as usize];
        new_state[3][col] = TABLE_11[state[0][col] as usize]
            ^ TABLE_13[state[1][col] as usize]
            ^ TABLE_9[state[2][col] as usize]
            ^ TABLE_14[state[3][col] as usize];
    }
